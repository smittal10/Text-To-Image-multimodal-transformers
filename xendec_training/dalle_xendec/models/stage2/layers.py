...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x, use_cache=False, layer_past=None,return_attention=False):
        B, T, C = x.shape
        ...
        if use_cache:
            if return_attention:
                return y.transpose(0, 1).contiguous(), present, attention  # (T, B, C) -> (B, T, C)
            else:
                return y.transpose(0, 1).contiguous() # (T, B, C) -> (B, T, C)
        else:
            if return_attention:
                return y.transpose(0, 1).contiguous(), attention  # (T, B, C) -> (B, T, C)
            else:
                return y.transpose(0, 1).contiguous()  # (T, B, C) -> (B, T, C)
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        # print("device inside forwards : {}, {}".format(x.device,return_attention.device))

        if return_attention:
            multi_attn, attention_probs = self.attn(self.ln1(x),return_attention=True)
            x = x + multi_attn
            x = x + self.mlp(self.ln2(x))
            return x, attention_probs
        else:
            multi_attn = self.attn(self.ln1(x))
            x = x + multi_attn
            x = x + self.mlp(self.ln2(x))
            return x
