...
class Transformer1d(nn.Module):
    ...
    def __init__(self,
        ...
        # transformer blocks
        self.blocks = nn.ModuleList([Block(ctx_len=hparams.ctx_len_img + hparams.ctx_len_txt,
    ...
    def forward(self,
                ...
                val=False) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        B, T = images.shape
        _, N = texts.shape
        ...
        for block in self.blocks:
            # print("block device {}".format(block.device))
            # print("x device {}".format(x.device))
            if not val:
                x, attention_probs = block(x,return_attention=True)
                # transpose attention probs if required----> expected shape [batch, target_length, source_length].
                per_layer_attn_probs.append(attention_probs)
            else:
                x = block(x)
        
        # Aggregate per-layer attention probs.
        if not val:
            aggregated_atten_probs = (torch.sum(torch.stack(per_layer_attn_probs)) / len(per_layer_attn_probs))
        ...
        if not val:
            return logits_img, logits_txt, aggregated_atten_probs
        else:
    ...
    def xendec_forward(self,
                images: torch.LongTensor,
                texts: torch.LongTensor,
                other_images: torch.LongTensor,
                other_texts: torch.LongTensor,
                pos_images: torch.LongTensor,
                pos_texts: torch.LongTensor,
                source_lambdas: torch.FloatTensor,
                image_lambdas:torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        ...
        ########################### other batch
        other_texts = self.tok_emb_txt(other_texts)
        other_images = self.tok_emb_img(other_images)

        ### interpolate with other batch #######
        source_lambdas = [torch.unsqueeze(a, -1) for a in source_lambdas]
        image_lambdas = [torch.unsqueeze(a, -1) for a in image_lambdas]
        ...
        text_embs = source_lambdas[0] * texts + source_lambdas[1] * other_texts
        text_embs += self.pos_emb_txt(pos_texts)
        image_embs = image_lambdas[0]*images + image_lambdas[1] * other_images
        image_embs += self.pos_emb_img(pos_images)
        x = torch.cat([text_embs, image_embs], axis=1).contiguous()
