...
from negate_dataset import CocoDataset
from torch.utils.data import DataLoader


parser = argparse.ArgumentParser()
parser.add_argument('-n', '--num_candidates', type=int, default=24)
...
def gen_save(captions,tokens,args,model,model_clip,path_to_save,map_fp,indx):
    B = tokens.shape[0]
    ...
    images = images.reshape(B,args.num_candidates,256,256,3)
...
coco = CocoDataset(args.caption_original_filtered,args.caption_negated_filtered,model.tokenizer)
valid_loader = DataLoader(dataset=coco,batch_size=5,shuffle=False,num_workers=4)
...
for global_i, (captions1,captions2,tokens1,tokens2) in enumerate(valid_loader):
    gen_save(captions1,tokens1,args,model,model_clip,model_dir_orig,fp_orig,global_i)
    gen_save(captions2,tokens2,args,model,model_clip,model_dir_bt,fp_bt,global_i)
    if global_i%5 == 0:
        fp_orig.flush()
        fp_bt.flush()
fp_orig.close()
