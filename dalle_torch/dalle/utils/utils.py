...
@torch.no_grad()
def clip_score_batched(prompt: List[str],
               images: np.ndarray,
               model_clip: torch.nn.Module,
               preprocess_clip,
               num_candidates: int,
    ...
    images = torch.stack(images, dim=0).to(device=device)
    texts = clip.tokenize(prompt).to(device=device)
    texts = torch.repeat_interleave(texts, num_candidates, dim=0)
    ...
    ## reshape and then sort per caption
    scores = scores.reshape(B,num_candidates)
    rank = torch.argsort(scores, descending=True,dim=1).cpu().numpy()
    return rank
